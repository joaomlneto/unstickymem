...
static const char *loglevel_colors[] = { "\033[37m",         // trace: white
		"\033[36m",         // debug: cyan
		"\033[32m",         // info: green
...
public:
	...
	inline void printHeaderRow() {
		printf("\033[1m\033[41m%-15s %-6s %5s %27s %-28s %-74s\033[m\n", "TIME",
				"PID", "LEVEL", "FILENAME:LINE", "FUNCTION", "MESSAGE");
	}
...
inline void log(LogLevel lvl, const char *fmt, const Args &... args) {
	...
	printf("\033[2m%s \033[4%dm%6d\033[m %s%5s\033[m ", time_str, getpid() % 8,
			getpid(), loglevel_colors[lvl], lvl._to_string());
	printf(fmt, args...);
	printf("\n");
}
