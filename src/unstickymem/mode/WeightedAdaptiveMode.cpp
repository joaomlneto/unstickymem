...
static Mode::Registrar<WeightedAdaptiveMode>
  registrar(WeightedAdaptiveMode::name(), WeightedAdaptiveMode::description());
...
void WeightedAdaptiveMode::adaptiveThread() {
  ...
  for (uint64_t local_percentage = (100 / numa_num_configured_nodes() + 4) / 5 * 5;
    ...
    stall_rate = get_average_stall_rate(_num_polls, _poll_sleep, _num_poll_outliers);
    ...
    if (stall_rate > best_stall_rate * 1.001) {
      ...
      if (get_average_stall_rate(_num_polls * 2, _poll_sleep, _num_poll_outliers * 2)) {
        LINFO("I guess so!");
        break;
      }
    }
