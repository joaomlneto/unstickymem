...
MemoryMap::MemoryMap() {
	...
	while (getline(&line, &line_size, maps) > 0) {
		...
		if (s.name() == "[heap]") {
			...
			_segments->emplace_back(s.startAddress(), s.endAddress(),
					"anonymous");
		}
	}
...
MemoryMap& MemoryMap::getInstance(void) {
	...
	if (!object) {
		...
		void *buf = WRAP(mmap)(nullptr, sizeof(MemoryMap),
				PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
		DIEIF(buf == MAP_FAILED,
				"error allocating space for memory map object");
...
int MemoryMap::handle_posix_memalign(void **memptr, size_t alignment,
	...
	void *start = *memptr;
	void *end = reinterpret_cast<void*>(reinterpret_cast<intptr_t>(*memptr)
			+ size - 1);
