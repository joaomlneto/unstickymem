...
void *realloc(void *ptr, size_t size) {
	...
	// dont do anything fancy if library is not initialized
	if (!unstickymem::is_initialized || inside_unstickymem) {
		return ((void *(*)(void*, size_t)) dlsym(RTLD_NEXT, "realloc"))(ptr,
				size);
	}
...
void *reallocarray(void *ptr, size_t nmemb, size_t size) {
	...
	// dont do anything fancy if library is not initialized
	if (!unstickymem::is_initialized || inside_unstickymem) {
		return ((void *(*)(void*, size_t, size_t)) dlsym(RTLD_NEXT,
...
int posix_memalign(void **memptr, size_t alignment, size_t size) {
	...
	if (!unstickymem::is_initialized) {
		return ((int (*)(void**, size_t, size_t)) dlsym(RTLD_NEXT,
				"posix_memalign"))(memptr, alignment, size);
	}
	// handle the function ourselves
	int result = unstickymem::memory->handle_posix_memalign(memptr, alignment,
			size);
	LTRACEF("posix_memalign(%p, %zu, %zu) => %d", memptr, alignment, size,
			result);
	return result;
}
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd,
	...
	void *result = unstickymem::memory->handle_mmap(addr, length, prot, flags,
			fd, offset);
	LTRACEF("mmap(%p, %zu, %d, %d, %d, %d) => %p", addr, length, prot, flags,
			fd, offset, result);
	// return the result
	return result;
}
...
int munmap(void *addr, size_t length) {
	// dont do anything fancy if library is not initialized
	if (!unstickymem::is_initialized) {
		return ((int (*)(void*, size_t)) dlsym(RTLD_NEXT, "munmap"))(addr,
				length);
	}
	// handle the function ourselves
...
long mbind(void *addr, unsigned long len, int mode,
	...
	if (!unstickymem::is_initialized) {
		return ((long (*)(void*, unsigned long, int, const unsigned long*,
				unsigned long, unsigned)) dlsym(RTLD_NEXT, "mbind"))(addr, len,
				mode, nodemask, maxnode, flags);
	}
	// handle the function ourselves
