...
MemorySegment::MemorySegment(char *line) {
	...
	DIEIF(
			sscanf(line, "%lx-%lx %7s %lx %u:%u %lu %n%*[^\n]%n", &addr_start,
					&addr_end, perms_str, &_offset, &_deviceMajor,
					&_deviceMinor, &_inode, &name_start, &name_end) < 7,
			"FAILED TO PARSE");
...
void MemorySegment::print() {
	...
	snprintf(info, sizeof(info),
			...
			_offset, _deviceMajor, _deviceMinor, _inode,
			(isPrivate() ? 'P' : 'S'), (isExecutable() ? 'X' : '-'),
			(isWriteable() ? 'W' : '-'), (isReadable() ? 'R' : '-'),
			_name.c_str());
