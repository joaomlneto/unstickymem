...
void * doWork(void *arg) {
  ...
  unsigned long assigned_core = data->assigned_core;
  ...
  for (int i=0; i < 10; i++) {
    ...
    uint64_t bytes = bench_seq_read(memory_to_access, memory_size, bench_time, thread_no);
    nb_bytes_processed[thread_no] = bytes;
  ...
  throughput = ((double) nb_bytes_processed[thread_no] / 1024. / 1024.)
    / ((double) length / 1000000.);
  
  //aggregate cores throughput
  pthread_mutex_lock(&running_mutex);
  node_bw[numa_node_of_cpu(assigned_core)] += throughput;
  pthread_mutex_unlock(&running_mutex);
  
  printf("%lu\t%.2f\n", assigned_core, throughput);
