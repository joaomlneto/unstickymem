...
/* 
This function requires the following steps a priori:
1) installing the PMC kernel module:
cd pmc/pmcdriver/
sudo ./install.sh

2) setting the right hw perf counter to be monitored:
cd pmc/pmctest
./c64.sh
./a.out startcounters 111

(They suggest we wait a few seconds before running benchmarks)
 */

void measure_stalls() {
  const int pmc_num = 0x00000000; // program monitor counter number
  static uint64_t prev_clockcounts = 0;
  static uint64_t prev_pmccounts = 0;
  ...
  if (prev_clockcounts > 0 )
    printf("==============\nHw perf: \nclocks since last measurement: %ld\nMeasured stalls: %ld\nstall rate: %f\n\n",
           (clock-prev_clockcounts), (pmc-prev_pmccounts), ((float)(pmc-prev_pmccounts))/(clock-prev_clockcounts));
  else
    printf("Setting up hw perf statistics for the first time\n");
...
void printmask(const char *name, struct bitmask *mask) {
  ...
  for (int i = 0; i < mask->size; i++)
    if (numa_bitmask_isbitset(mask, i)) printf("%d ", i);
  putchar('\n');
}

void place_pages(uint64_t *memory_to_access, uint64_t memory_size, double ratio) {
  int r;
...
void * doWork(void *pdata) {
  ...
  **/
  ...
  for (double w1=0.50; w1<=1.00; w1+=0.10) {
    printf("[w1=%lf, w2=%lf]: starting new iteration..............\n", w1, 1-w1);
    place_pages(memory_to_access, memory_size, w1);
    if (tn->thread_no == 0) measure_stalls();
    ...
    printf("[w1=%lf, w2=%lf]: iteration concluded in %ld seconds\n", w1,1-w1,length);
  }
